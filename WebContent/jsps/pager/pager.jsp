...
<script type="text/javascript">
	function _go() {
		...
		location = "${pb.url}&pc=" + pc;//表示超链接的地址
	}
</script>
...
<c:choose>
    ...
    <%--我们需要计算页码列表的开始和结束为止，即两个变量begin和end，计算他们需要通过当前页码
    1.总页数不足六页：begin=1，end=最大页
    2.通过公式计算begin和end，begin=当前页-2，end=当前页+3
    3.如果begin<1，那么令begin=1，end=6
    4.如果end>最大页，令end=最大页，begin=最大页-5
     --%>
...
<c:forEach begin="${begin }" end="${end }" var="i">
	<c:choose>
		<c:when test="${i eq pb.pc }">
			<span class="spanBtnDisabled">${i }</span>
		</c:when>
		<c:otherwise>
