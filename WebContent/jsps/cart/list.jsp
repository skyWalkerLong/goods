...
//给减号添加click事件
	...
	if(quantity == 1) {
		if(confirm("您是否真要删除该条目？")) {
			location = "/goods/CartItemServlet?mark=batchDelete&cartItemIds=" + id;
		}
	} else {
 ...
 */
 ...
 function batchDelete() {
 	...
 	location = "/goods/CartItemServlet?mark=batchDelete&cartItemIds=" + cartItemIdArray;
 }
...
function sendUpdateQuantity(id, quantity) {
	$.ajax({
		...
		data:{"mark":"updateQuantity",cartItemId:id,quantity:quantity},
		type:"POST",
		dataType:"json",
		...
		<c:forEach items="${cartItemList }" var="cartItem">
			<tr align="center">
				...
				<td align="left" width="70px">
					<a class="linkImage" href="<c:url value='/BookServlet?mark=load&bid=${cartItem.book.bid }'/>"><img border="0" width="54" align="top" src="<c:url value='/${cartItem.book.image_b }'/>"/></a>
				</td>
				<td align="left" width="400px">
				    <a href="<c:url value='/BookServlet?mark=load&bid=${cartItem.book.bid }'/>"><span>${cartItem.book.bname }</span></a>
				</td>
				...
				<td>
					<a href="<c:url value='/CartItemServlet?mark=batchDelete&cartItemIds=${cartItem.cartItemId }'/>">删除</a>
				</td>
			</tr>
		</c:forEach>
			...
			<form id="jieSuanForm" action="<c:url value='/CartItemServlet'/>" method="post">
				...
				<input type="hidden" name="mark" value="loadCartItems"/>
